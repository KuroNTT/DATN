...
export class ChangePwComponent {
  ...
  isValid(): boolean {
    ...
    if (!passwordRegex.test(passnew1)) {
      this.shakeField('passnew1', 'Mật khẩu phải có ít nhất 8 ký tự, chữ hoa và số');
      return false;
    }
  ...
  shakeField(field: 'pass_old' | 'passnew1' | 'passnew2', message: string): void {
    if (field === 'pass_old') {
  ...
  doipass() {
    ...
    fetch('http://localhost:3000/api/user/profile/change-pw', opt)
      .then(res => res.json())
      .then(data => {
